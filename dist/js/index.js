...
wayfinderApp.controller('WayfinderCtrl', [
  ...
  'wfangular3d',
  'tabsService',
  function($scope, $uibModal, $log, $timeout, $rootScope, $sce, $http,
        wayfinder, tabsService) {
        var kbLayoutEt = {
            "name": "Estonian",
            "local_name": "Eesti",
            "lang": "et",
        ...
        var searchKeyboard = new Keyboard($(".search-keyboard"),
        ...
        $scope.showGroup = function(group) {
            ...
            for (var key1 in kioskFloorPOIs) {
                ...
                for (var key2 in kioskFloorPOIsPOIGroups) {
                    if (kioskFloorPOIsPOIGroups[key2].getName(
                            wayfinder.getLanguage()) === group.getName(
                            wayfinder.getLanguage())) {
                        poiHasGroup = 1;
                    };
                };
            ...
            if (pois.length == 0) {
                ...
                pois = group.getPOIs();
                console.log(
                    "showing way to nearest in whole building");
            };
            console.log(pois);
            if (pois == []) {
...
wayfinderApp.controller('ModalCtrl', [
  ...
  'wayfinder',
  'tabsService',
  function($rootScope, $scope, $uibModalInstance, poi, language, wayfinder,
        service) {
    ...
    return {
        ...
        link: function(scope, element, attr) {
            ...
            scope.$watch('lastButton', function() {
                var allButtons = $(element).find(
                    'floor-button');
                //check the maximum width of element
                angular.forEach(allButtons, function(ele,
                    ind) {
                    //below code will find maxWidth
                    maxWidth = ele.innerWidth >
                        maxWidth ? ele.innerWidth :
                        maxWidth;
                });
